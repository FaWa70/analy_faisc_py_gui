...
from src.fitting_module_v2 import gauss2D_cst_offs, sot_r_gau
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.title = "V2 - This analyzes peaked beam profiles (64-image wcf file, or similar images in folder)"
...
class MyTableWidget(QWidget):
    def __init__(self, parent):
        ...
        def design_tab2():
            ...
            self.cob_use_mean.addItems(["Use mean from ROI", "Use Aeff-curve fit"])
            ...
            self.LbTitleSub = QLabel('For Aeff-curve and horizontality fit')
            ...
            self.LbAeffTitle = QLabel('Effective beam surface (Aeff):')
            ...
            # Aeff display
            self.LbAeffg = QLabel('Aeff : XX px')
            ...
            self.TbFit2.setVerticalHeaderLabels(['Make fit?', 'Use for Aeff \n(if activated above)',
            ...
            def fill_fit_tb():
                # col 0
                """LbYesNo = QLabel()
                self.TbFit2.setCellWidget(0, 0, LbYesNo)
                LbFLim = QLabel('Lower fit limit (rel.)')
                self.TbFit2.setCellWidget(1, 0, LbFLim)
                LbRdBtG = QLabel('Use for Aeff \n(if activated above)')
                self.TbFit2.setCellWidget(2, 0, LbRdBtG)
                LbFStatus = QLabel('status')
                self.TbFit2.setCellWidget(3, 0, LbFStatus)
                LbGOF = QLabel('Which GOF?')
                self.TbFit2.setCellWidget(4, 0, LbGOF)
                LbFw1 = QLabel('w_1 (px)')
                self.TbFit2.setCellWidget(5, 0, LbFw1)
                lb_f_max = QLabel('Max val (rel.)')
                self.TbFit2.setCellWidget(6, 0, lb_f_max)
                Lbx0 = QLabel('x_0 (px)')
                self.TbFit2.setCellWidget(7, 0, Lbx0)
                Lby0 = QLabel('y_0 (px)')
                self.TbFit2.setCellWidget(8, 0, Lby0)
                LbFw2 = QLabel('w_2 (px)')
                self.TbFit2.setCellWidget(9, 0, LbFw2)
                LbFteta = QLabel('Angle (°)')
                self.TbFit2.setCellWidget(10, 0, LbFteta)"""
        ...
        def design_tab3():
            ...
            self.tb_res3 = QTableWidget(6, 0)
            tab3_lower_HB_lay.addWidget(self.tb_res3)
            self.tb_res3.setVerticalHeaderLabels(['Export?', 'Mean', 'Rel err.', 'Std err.', 'Min.', 'Max.', 'Median'])

            # Export
            self.btExport4 = QPushButton('Export')
            # self.btExport4.clicked.connect(self.)
    ...
    def Load_File1(self):
        ...
        self.LbFolderName.setText(f_name)
    ...
    def make_fits(self):
        ...
        for col_idx in range(self.TbFit2.columnCount()):
            ...
            if col_idx == 0:  # col_idx in [0, 2]  # gauss2D_cst_offs
            ...
            elif col_idx == 2:  # elliptic gau
                pass
                # b_fits[0][col_idx]["w2"] = float(self.TbFit2.cellWidget(8, col_idx).text())
                # b_fits[0][col_idx]["angle"] = float(self.TbFit2.cellWidget(9, col_idx).text())
    ...
    def export_graphs(self):
        ...
        # print(dt.datetime())
        ws1.cell(row=1, column=3).value = 'Analyzed on: ' # + str(dt.datetime())
        ...
        imNoS = np.array(range(1, 64 + 1))  # x-axis
        for imNo in imNoS:
            ws1.cell(row=imNo - 1 + 9, column=1).value = imNo

        def make_leg_entry(line):
            entry = ""
            if self.TbTasks.cellWidget(line, 1).currentText() == "Full frame":
                entry += "full NA "
            else:
                if self.TbTasks.cellWidget(line, 1).currentText() == "Roi 1":
                    entry += "roi1"
                else:
                    entry += "roi2"
                # outside?
                if self.TbTasks.cellWidget(line, 2).currentText() == "Outside":
                    entry += " out"
                else:
                    entry += " in "

            # add masking info
            if self.TbTasks.cellWidget(line, 3).currentText() == "Ignore masked pixels":  # remove masked pixels from ma
                entry += " mas"
            else:
                entry += " all"

            # add function info
            if self.TbTasks.cellWidget(line, 4).currentText() == "Mean Value":  # put in loop if it does not work here
                entry += " mea"
            elif self.TbTasks.cellWidget(line, 4).currentText() == "Max Value":
                entry += " max"
            elif self.TbTasks.cellWidget(line, 4).currentText() == "Min Value":
                entry += " min"
            return entry

        for line in range(self.tablines):  # loop through lines of table
            if self.TbTasks.cellWidget(line, 6).currentText() == "No":  # do not export this data set
                continue
    ...
    def make_auto_crop(self):  # only for the stack imge
        ...
        if not self.cb_no_auto_crop.isChecked() \
                and (self.cob_roi.currentText() == "Outside ROI"
                     or self.cob_use_mean.currentText() == "Use Aeff-curve fit"):
    ...
    def get_backg(self):
        ...
        elif self.cob_use_mean.currentText() == "Use Aeff-curve fit":  # get offset from Aeff-curve fit
    ...
    def get_aeffg(self):
        ...
        print('Aeffg = ', Aeffg)
        self.LbAeffg.setText('A_eff : {:.2f} px.'.format(Aeffg))
    ...
    def get_step(self, minstep=2):
        ...
        if ((self.cob_use_mean.currentText() == "Use mean from ROI"  # is there a reasonable frame ?
             and self.cob_roi.currentText() == "Outside ROI") or
                (self.cob_use_mean.currentText() == "Use Aeff-curve fit")):
    ...
    def plot2_1D(self):
        ...
        if self.Cob1Ddisp.currentText() == "Cross sections":
            ...
            # calculate model data (slow version) 1-3 ms
            ti_mod = ti.time()
            XX, YY = np.meshgrid(range(im_to_show2.shape[1]), range(im_to_show2.shape[0]))
            ...
            # plot model data: vertical cross section
            c_section = mod_z[:, int(round(b_max[0]))]
            axPlt2.plot(range(im_to_show2.shape[0]), c_section, '--g', label="round G. vert.")
            # horizontal cross section
            c_section = mod_z[int(round(b_max[1])), :]
            axPlt2.plot(range(im_to_show2.shape[1]), c_section, '-.', color="red", label="round G. horiz.")
            print("model plots took {:f} s".format(ti.time() - ti_mod))

            # Add the same for elliptic Gaussian
        ...
        elif self.Cob1Ddisp.currentText() == "Aeff curves":
    ...
    def make_result_dict(self):
        ...
        if ((self.TbFit2.cellWidget(0, 1).currentText() == "Yes") or
                (self.TbFit2.cellWidget(0, 2).currentText() == "Yes") or
                (self.TbFit2.cellWidget(0, 3).currentText() == "Yes")):
        ...
        # Fill the results-dict using the values above and define the jeys at the same time
        # The keys will be used as headers of the Excel sheet and the cobos fpr plotting the histograms
        ...
        # TODO: make a list of column header that do ot need to appear in the table
        # TODO: make decimal point justification
        # TODO: adjust col width to header
        ...
        for col_idx in range(len(key_li)):  # the names can be reused but new instances need to be created
            ...
            the_mean = results[key_li[col_idx]].mean()
            LbResMean = QLabel('{:.2f}'.format(the_mean))
            self.tb_res3.setCellWidget(1, last_col_idx, LbResMean)

            confidence = 0.6827
            abs_std_err_of_mean = (sem(results[key_li[col_idx]])
                                   * t.ppf((1 + confidence) / 2,
                                           df=(len(results[key_li[col_idx]]) - 1)))
            # PPF (Percent point function) is the inverse of the CDF. That is, PPF gives the value of the variate
            # for which the cumulative probability has the given value: (1-c)/2 + C = (1+c)/2
            ...
            LbResMin = QLabel('{:.2f}'.format(results[key_li[col_idx]].min()))
            self.tb_res3.setCellWidget(4, last_col_idx, LbResMin)

            LbResMax = QLabel('{:.2f}'.format(results[key_li[col_idx]].max()))
            self.tb_res3.setCellWidget(5, last_col_idx, LbResMax)

            LbResMed = QLabel('{:.2f}'.format(np.median(results[key_li[col_idx]])))
            self.tb_res3.setCellWidget(6, last_col_idx, LbResMed)
        # cobo, 'Mean', 'rel err.', 'std err.', 'Min.', 'Max.', 'Median'
    ...
    def plot3_1D(self):
        ...
        axPlt3 = self.figurePlt3.add_subplot(221)
        axPlt4 = self.figurePlt3.add_subplot(222)
        axPlt5 = self.figurePlt3.add_subplot(223)
        axPlt6 = self.figurePlt3.add_subplot(224)
        axPlt_li = [axPlt3, axPlt4, axPlt5, axPlt6]
        col_li = ['green', 'grey', 'red', 'blue']
        # label and x_axis label  will be combo box text
        CobPl_li = [self.CobPl31, self.CobPl32, self.CobPl33, self.CobPl34]

        # cols in res_arr : nb of image, Aeff, slope, b_max[2], backg
        ...
        for (this_ax, this_col, this_cobo) in zip(axPlt_li, col_li, CobPl_li):
            ...
            if "POS" in curr_text.upper():  # make a depointing plot
                ...
                this_ax.hist(results[this_cobo.currentText()],
                             color=this_col,
                             label=this_cobo.currentText(),
