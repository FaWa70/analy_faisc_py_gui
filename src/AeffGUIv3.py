...
class MyTableWidget(QWidget):
    ...
    def export(self):
        ...
        ws2.cell(row=2, column=5).value = len(good_idx)

        # write first column indicating the meaning of the first lines
        str_li = ['Mean', 'Rel err.', 'Std err.', 'Min.', 'Max.', 'Median']
        for idx in range(len(str_li)):
            ws2.cell(row=4+idx, column=1).value = str_li[idx]


        # continue with empty line and then the header in row 11
        ws2.append([""])
        key_li = list(results.keys())
        ws2.append(key_li)
        # Data in row 12 and below
        # (row by row so later it may be changed to faster version)
        for li_idx in range(len(good_idx)):
            li_list = []
            for col_idx in range(len(key_li)):
                li_list.append(results[key_li[col_idx]][li_idx])
