...
class Window(QWidget):
    ...
    def AeffCurveBasic(offset, image, points=5, step=5, PixSurface=1, imBackground='absent'):
        ...
        # points=5 # default of optional parameter, but should be passed
        #    should be the same as in calling function
        # step=5 # default of optional parameter, but should be passed
        #    should be the same as in calling function
        # PixSurface=1 # default of optional parameter (if =1, Aeff is in pixels)
        #    should be the same as in calling function
        # imBackground = 'absent' # default of optional parameter
    ...
    def mertit_Aeff_slope_Sq(offset, image, points=5, step=5, PixSurface=1, imBackground='absent'):
        ...
        Aeffs = main.AeffCurveBasic(offset, image, points, step, PixSurface, imBackground)
    ...
    def Calc_Aeff(self):
        ...
        # Correspond à la figure 2 de version 1
        fig4 = plt.figure() #create figure object (define fig size)
        ax4 = fig4.add_subplot(111) # add axes (one row , one col, plot num 1)
        # ax2.plot(ImSize,AeffS, 'ob', ms=5, label='first run')
        ax4.set_xlabel('Square pixels in software aperture');
        ax4.set_ylabel('$A_{eff}$');


        # Check the sub function and show the primary good data range
        #def AeffCurveBasic(offset, image, points=5, step=5, PixSurface=1, imBackground='absent'):
        GoodAeffs = main.AeffCurveBasic(0, image, len(AeffS)-minIdx, step, PixSurface)
        GoodImSiz = ImSize[minIdx:]
        ax4.plot(GoodImSiz,GoodAeffs, 'ro', ms=5, markerfacecolor='none', label='Good points')
        ...
        if minIdx > len(AeffS)-3:
            ...
            fitRes = opt.minimize(main.mertit_Aeff_slope_Sq, x0=(BruitMoyen), args=(image, useSteps, step, PixSurface) )

            # montrer et sortir le resultat
            # print('Best noise value = ', fitRes.x[0])
            GoodAeffs = main.AeffCurveBasic(fitRes.x[0], image, memCo, step, PixSurface)
            GoodImSiz = ImSize[-memCo:]
            ax4.plot(GoodImSiz,GoodAeffs, '*g', ms=5, label='After offset correction')
            ax4.plot(GoodImSiz[-useSteps:],GoodAeffs[-useSteps:], '*r', ms=5, label='points used for fit')
            ...
            ax4.plot([0, ImSize.max()],[droiteParS[0], droiteParS[1]*ImSize.max()+droiteParS[0]], '--k', label='result')
            ax4.text(GoodImSiz[-useSteps],GoodAeffs[-useSteps]*1.1,'A_eff = {:.1f}'.format(GoodAeffs[-useSteps]))
            ax4.legend(loc='best')
        ...
        print(' ')
        print('Processing time was: ',ti.perf_counter() - t1, " s")
        print('This was version 2 (fast) with 1 plot')
        print('------------------------')

        # random data
        data = [np.random.random() for i in range(10)]

        # destroys the ax object, then there is no error message any more
        self.figureAFC.clear()

        # create an axis
        ax = self.figureAFC.add_subplot(111)
