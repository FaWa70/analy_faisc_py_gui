"""
@author: Frank WAGNER, Reison GJACI,
"""
...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget,
                             ...
                             QSpinBox, QDoubleSpinBox, QProgressBar, QComboBox,
                             QTableWidget, QTableWidgetItem,
                             QSizePolicy, QTextEdit)
from PyQt5.QtCore import Qt, QRectF, QUrl  # for the focus from keyboard
from PyQt5.QtGui import QFont, QFontMetricsF, QIcon, QKeySequence
...
def extract_z_from_filename_for_tiff(fi):
    z = -20e20
    # extract the position floating point number from the string fi
    rr = re.findall(r"[-+]?[.]?[mp]?[\d]+(?:,\d\d\d)*[\.|\,|\_]?\d*(?:[eE][-+]?\d+)?", fi)
    zz = rr[-1]  # The last number should be the z-position, other numbers may follow and are ignored
    zz = zz.replace("_", ".")
    zz = zz.replace(",", ".")
    zz = zz.replace("p", "+")
    zz = zz.replace("m", "-")
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        global py_file_folder
        py_file_folder = os.getcwd()
...
class MyTableWidget(QWidget):
    def __init__(self, parent):
        ...
        def add_widgets_to_tab1():  # Create widgets of 1st tab : inspect
            ...
            self.SbImFrom.setMinimum(0)
            self.SbImFrom.setMaximum(1000)
            ...
            self.SbImTo.setMinimum(0)
            self.SbImTo.setMaximum(1000)
        ...
        def design_tab2():
            ...
            self.LbSaturation = QLabel('Not saturated')
            tab2_upper_grid2.addWidget(self.LbSaturation, 3, 4, 1, 2)
            ...
            self.Cob2Ddisp.addItems(["Measurement", "Round G. model",
                                     "Round G. residual (measurement - model)",
                                     "Elliptic G. model",
                                     "Elliptic G. residual (measurement - model)",
                                     "Surface over fixed threshold (SOT)"])
            ...
            # Add the SOT grid
            # Title
            sot_grid_lay = QGridLayout()
            self.LbSotTitle = QLabel('Surface over fixed threshold (SOT):')
            self.LbSotTitle.setFont(section_h_font)
            sot_grid_lay.addWidget(self.LbSotTitle, 0, 0, 1, 2)

            # Switch this calculation on and off
            self.cbMakeSotCalc = QCheckBox('Calculate the SOT for a fixed, relative threshold')
            self.cbMakeSotCalc.stateChanged.connect(self.onImageChange)
            sot_grid_lay.addWidget(self.cbMakeSotCalc, 1, 0, 1, 2)

            # Label for the threshold value
            self.LbSotThresh = QLabel('Use a rel. threshold of:')
            sot_grid_lay.addWidget(self.LbSotThresh, 2, 0)

            # The value of the threshold in a double spinbox
            self.sb_sot_thresh = QDoubleSpinBox()
            self.sb_sot_thresh.setMinimum(0.0000)
            self.sb_sot_thresh.setMaximum(1.0000)
            self.sb_sot_thresh.setValue(0.135)
            self.sb_sot_thresh.setDecimals(3)
            self.sb_sot_thresh.setSingleStep(0.05)
            self.sb_sot_thresh.valueChanged.connect(self.onImageChange)
            sot_grid_lay.addWidget(self.sb_sot_thresh, 2, 1)

            # The display of the result
            self.LbSotResult = QLabel('The SOT is: XX px²')
            sot_grid_lay.addWidget(self.LbSotResult, 3, 0)

            right_wi_lay.addLayout(sot_grid_lay)
            ...
            def fill_fit_tb():
                ...
                for col_idx in range(3):  # the names can be reused but new instances need to be created
                    ...
                    rdbt_for_aeff.setStyleSheet('QRadioButton::indicator { width: 50px;};')
                    ...
                    """for x in [cob_YesNo, ed_f_lim, ed_f_w1, ed_f_max]:
                        if col_idx == 0:
                            x.setFixedWidth(65)
                        if col_idx == 1:
                            x.setFixedWidth(80)
                ...
                """ed_f_w2.setFixedWidth(45)"""

                ed_teta = QLineEdit('0')
                self.TbFit2.setCellWidget(10, 2, ed_teta)
                """ed_teta.setFixedWidth(45)"""

                self.TbFit2.resizeRowsToContents()
                self.TbFit2.resizeColumnsToContents()
                self.TbFit2.setStyleSheet(""" 
                                    * {padding-right: 5px; 
                                       selection-background-color: lightgray} """)


            fill_fit_tb()
            #table_model = PandasModel(self.read_to_b_fits)
            #self.TbFit2.setModel(table_model)
            self.TbFit2.resizeColumnsToContents()
            self.TbFit2.resizeRowsToContents()
            right_wi_lay.addWidget(self.TbFit2)
        ...
        def design_tab3():
            ...
            self.Cob4Pl31 = QComboBox()
            tab3_cobo_grid_lay.addWidget(self.Cob4Pl31, 0, 0)
            self.Cob4Pl31.activated.connect(self.plot3_1D)

            self.Cob4Pl32 = QComboBox()
            tab3_cobo_grid_lay.addWidget(self.Cob4Pl32, 0, 1)
            self.Cob4Pl32.activated.connect(self.plot3_1D)

            self.Cob4Pl33 = QComboBox()
            tab3_cobo_grid_lay.addWidget(self.Cob4Pl33, 1, 0)
            self.Cob4Pl33.activated.connect(self.plot3_1D)

            self.Cob4Pl34 = QComboBox()
            tab3_cobo_grid_lay.addWidget(self.Cob4Pl34, 1, 1)
            self.Cob4Pl34.activated.connect(self.plot3_1D)
            ...
            self.tb_res3.installEventFilter(self)

            # Export
            self.btExport3 = QPushButton('Export')
            tab3_lower_HB_lay.addWidget(self.btExport3)
            self.btExport3.clicked.connect(self.export3)
        ...
        def design_tab4():
            ...
            # Choose data origin # begin
            tab4_data_origin_grid_wid = QWidget()
            tab4_data_origin_grid_lay = QGridLayout(tab4_data_origin_grid_wid)

            self.LbChoseDataOrigin = QLabel("Choose data origin:")  # label
            tab4_data_origin_grid_lay.addWidget(self.LbChoseDataOrigin, 0, 0, 1, 1)

            self.CobDataOrigin = QComboBox()  # combobox
            self.CobDataOrigin.addItems(["Process image files with identical parameters",
                                         "Load data from Excel files exported earlier"])
            tab4_data_origin_grid_lay.addWidget(self.CobDataOrigin, 0, 1, 1, 3)
            self.CobDataOrigin.activated.connect(self.set_checkb_state4_2)

            self.btDataOrigin = QPushButton("Get data now!")  # combobox
            tab4_data_origin_grid_lay.addWidget(self.btDataOrigin, 1, 1, 1, 2)
            self.btDataOrigin.clicked.connect(self.select_data_origin)
            tab4_ul_VB_lay.addWidget(tab4_data_origin_grid_wid)

            self.CbExport = QCheckBox("Export individual excel files")
            tab4_data_origin_grid_lay.addWidget(self.CbExport, 3, 1, 1, 4)
            # Choose data origin # end

            tab4_ul_VB_lay.addStretch(1)

            # Choose z-curves to plot # begin
            self.LbChosePlots = QLabel("Choose z-curves to plot")  # label
            tab4_ul_VB_lay.addWidget(self.LbChosePlots)

            tab4_cobo_grid_wid = QWidget()
            tab4_cobo_grid_lay = QGridLayout(tab4_cobo_grid_wid)

            self.Cob4Pl1 = QComboBox()
            tab4_cobo_grid_lay.addWidget(self.Cob4Pl1, 0, 0)
            self.Cob4Pl1.activated.connect(self.set_checkb_state4)
            self.Cb4Pl1 = QCheckBox("Use eff. radius?")
            # self.Cb4Pl1.setEnabled(False)
            tab4_cobo_grid_lay.addWidget(self.Cb4Pl1, 1, 0)
            self.Cb4Pl1.stateChanged.connect(self.set_checkb_state4)

            self.Cob4Pl2 = QComboBox()
            tab4_cobo_grid_lay.addWidget(self.Cob4Pl2, 2, 0)
            self.Cob4Pl2.activated.connect(self.set_checkb_state4)
            self.Cb4Pl2 = QCheckBox("Use eff. radius?")
            # self.Cb4Pl2.setEnabled(False)
            tab4_cobo_grid_lay.addWidget(self.Cb4Pl2, 3, 0)
            self.Cb4Pl2.stateChanged.connect(self.set_checkb_state4)

            self.Cob4Pl3 = QComboBox()
            tab4_cobo_grid_lay.addWidget(self.Cob4Pl3, 4, 0)
            self.Cob4Pl3.activated.connect(self.set_checkb_state4)
            self.Cb4Pl3 = QCheckBox("Use eff. radius?")
            # self.Cb4Pl3.setEnabled(False)
            tab4_cobo_grid_lay.addWidget(self.Cb4Pl3, 5, 0)
            self.Cb4Pl3.stateChanged.connect(self.set_checkb_state4)

            self.Cob4Pl4 = QComboBox()
            tab4_cobo_grid_lay.addWidget(self.Cob4Pl4, 6, 0)
            self.Cob4Pl4.activated.connect(self.set_checkb_state4)
            self.Cb4Pl4 = QCheckBox("Use eff. radius?")
            # self.Cb4Pl4.setEnabled(False)
            tab4_cobo_grid_lay.addWidget(self.Cb4Pl4, 7, 0)
            self.Cb4Pl4.stateChanged.connect(self.set_checkb_state4)

            tab4_ul_VB_lay.addWidget(tab4_cobo_grid_wid)
            # Choose z-curves to plot # end

            tab4_ul_VB_lay.addStretch(1)
            self.btExport4 = QPushButton("Export z-data to Excel file")
            tab4_ul_VB_lay.addWidget(self.btExport4)
            self.btExport4.clicked.connect(self.export4)
            tab4_ul_VB_lay.addStretch(1)
            ...
            self.figurePlt4 = Figure()
            self.canvasPlt4 = FigureCanvas(self.figurePlt4)
            tab4_ur_VB_lay.addWidget(self.canvasPlt4)
            self.toolbarPlt4 = NavigationToolbar(self.canvasPlt4, self)
            tab4_ur_VB_lay.addWidget(self.toolbarPlt4)
        ...
        def design_tab5():
            ...
            # This does not work:
            # self.te_short_manual.loadResource(1, QUrl.fromLocalFile("tab5_documentation.html"))
            with open(os.path.join(py_file_folder, r"doc_files/tab5_documentation.html"), "r") as doc_file:
                lines = doc_file.readlines()
            doc_text = "\n".join(lines)
            self.te_short_manual.setHtml(doc_text)

            self.te_short_manual.setReadOnly(True)
            tab5_VB_lay.addWidget(self.te_short_manual)

            """
            https://doc.qt.io/qtforpython/PySide6/QtWidgets/QTextEdit.html 
            QTextEdit can display images, lists and tables. If the text is too large to view 
            within the text edit’s viewport, scroll bars will appear. The text edit can **load** 
            both plain text and rich text files. Rich text can be described using a subset of 
            HTML 4 markup; refer to the Supported HTML Subset page for more information.
            (https://doc.qt.io/qtforpython/overviews/richtext-html-subset.html#supported-html-subset)
            .loadResource(type, name)  # const QUrl &name
            type-Value	Description
        	0	No resource is loaded, or the resource type is not known.
            1	The resource contains HTML.
        	2	The resource contains image data. Currently supported data types are 
        	QVariant::Pixmap and QVariant::Image. If the corresponding variant is of type 
        	QVariant::ByteArray then Qt attempts to load the image using QImage::loadFromData. 
            3	The resource contains CSS.
        	4	The resource contains Markdown.
            """
    ...
    def eventFilter(self, source, event):
        if (event.type() == QtCore.QEvent.KeyPress and
            ...
            self.copy_selection()
            return True
        return super(MyTableWidget, self).eventFilter(source, event)  # returns event to parent


    # add copy method
    def copy_selection(self):
        selection = self.tb_res3.selectionModel().selectedIndexes()
        if selection:
            print("Copyselection: Selection detected")
            paste_text = ""
            for index in selection:
                paste_text = paste_text + self.tb_res3.cellWidget(index.row(), index.column()).text() + "\n"
            QApplication.instance().clipboard().setText(paste_text)
    ...
    def load_stack(self):
        """ Called by:
        Load_File, crop_or_uncrop; needs f_name global variable
        ...
        if analyze_all_files_check:
            main.statBar.showMessage(statusbarmessage+'\tReading images... please wait', 5000)
        else:
        ...
        if file_type.upper() == "WCF":  # read wcf file that contains 64 images (taken in identic cond.)
            ...
            std_image_reslS = {  # key in in kpixels, tuple in pixels
                ...
                int(np.round(384 * 384 / 1000)): (384, 384),
            ...
            try:
                ...
                sat_value = 10
                if analyze_all_files_check:
                    main.statBar.showMessage(statusbarmessage + '\tThe selected file is not readable')
                else:
                    main.statBar.showMessage('The selected file is not readable')
                self.LbFolderName.setText('The selected file is not a readable wcf-file')
            ...
            cm = 2  # pixel (crop margin, because the nominal image size contains a saturated pixel)
            ...
            try:  # check if the selected image works
                ...
                if analyze_all_files_check:
                    ...
                    main.statBar.showMessage('The selected file is not a readable image')
                self.LbFolderName.setText('The selected file is not a readable image or wcf-file')
                return
            # get the list of filenames
            dirname = os.path.normpath(os.path.split(f_name)[0])
        ...
        if analyze_all_files_check:
            main.statBar.showMessage(statusbarmessage+'\tFinished loading images', 5000)
        else:
    ...
    def UpdateLinesIm1(self, xM =-10, yM = -10):
        ...
        try:
            for line in axIM1.get_lines():  # ax.lines:
                line.remove()
    ...
    def make_fits(self):
        ...
        # print(b_fits)
        if analyze_all_files_check:
            main.statBar.showMessage(statusbarmessage + '\tFitting in progress (slow for elliptic Gaussian', 5000)
        else:
        ...
        for col_idx in range(self.TbFit2.columnCount()):
            ...
            # initialize the beam position with b_max data if launched from z-evolution
            if analyze_all_files_check:
                b_fits[0][col_idx]["x_0"] = b_max[0]
                b_fits[0][col_idx]["y_0"] = b_max[1]
        ...
        if analyze_all_files_check:
            main.statBar.showMessage(statusbarmessage + '\tFitting finished', 5000)
        else:
    ...
    def export3(self):
        ...
        if file_type == "wcf":
            ...
            if not analyze_all_files_check:  # launched by tab 3
    ...
    def export4(self):
        print("export4 called")
        z_series_wb = xl.Workbook()  # workbook
        ws = z_series_wb.active
        ws.title = "z-results"

        tod = dt.datetime.today()
        tod_str = "{:0d}-{:0d}-{:0d} {:0d}h{:0d}m{:0d}s".format(
            tod.year, tod.month, tod.day, tod.hour, tod.minute, tod.second)
        ws.cell(row=1, column=1).value = "File information: "
        ws.cell(row=2, column=2).value = "Location of treated files: "
        if file_type == "wcf":
            ws.cell(row=2, column=3).value = dirname
        else:  # other types
            ws.cell(row=2, column=3).value = dirname2

        ws.cell(row=3, column=2).value = "Analyzed on: "
        ws.cell(row=3, column=3).value = tod_str
        ws.cell(row=4, column=2).value = "File type: "
        ws.cell(row=4, column=3).value = file_type
        ws.cell(row=5, column=2).value = "File name: "
        ws.cell(row=5, column=3).value = baseName

        key_li = list(results.keys())
        # shown_li : global variable containing indices of results key_li
        ws.append([""])  # continue with empty line
        # prepare list of headers spaced by one empty col
        header_list = list(np.array(key_li)[shown_li])
        spaced_header_list = [""] * (2 * len(header_list) - 1)
        spaced_header_list[::2] = header_list
        ws.append(["nb of images", "z (mm?)"] + spaced_header_list)  # write headers on top of value lines
        ws.append(["", ""] + len(header_list) * ["mean", "std dev"])  # write headers on top of value lines
        ...
        for z_idx, res_dict in enumerate(final_results_list):
            ...
            for h_idx, header in enumerate(header_list):
                ...
                export_arr[z_idx, 2 * h_idx + 2] = res_dict[header].mean()
                export_arr[z_idx, 2 * h_idx + 3] = res_dict[header].std()

        # sort according to z
        export_arr = export_arr[export_arr[:, 1].argsort()]  # ?? .copy()
        for z_idx in range(len(final_results_list)):
            ws.append(list(export_arr[z_idx, :]))

        if file_type == "wcf":
            fpath = os.path.normpath((dirname + "/z-series " + tod_str + ".xlsx"))
        else:  # other types
            fpath = os.path.normpath((dirname2 + "/z-series " + tod_str + ".xlsx"))
        z_series_wb.save(fpath)

    def GetMousePlot3(self, event):
        """
        Handles the filtering of the data used for the filled bars, the summary table
        and excel output in the histograms.
        """
    ...
    def medFi_state_changed(self):
        ...
        if newBtIdx == 1:  # preview became active
            ...
            if analyze_all_files_check:
                main.statBar.showMessage(statusbarmessage + '\tProcessing ...  please wait', 5000)
            else:
            ...
            for Nb in range(1, imStack.shape[2] + 1):
                ...
                QApplication.processEvents()
            if analyze_all_files_check:
                main.statBar.showMessage(statusbarmessage + '\tFinished', 5000)
            else:
    ...
    def display_MedFi_preview(self):
        ...
        try:
            ...
            if self.SbMedFi.value() > 1:
                if analyze_all_files_check:
                    main.statBar.showMessage(statusbarmessage + '\tProcessing', 5000)
                else:
                    main.statBar.showMessage('Processing')
                ImDisp1 = ndi.median_filter(imStack[:, :, number - 1], self.SbMedFi.value())
                if analyze_all_files_check:
                    main.statBar.showMessage(statusbarmessage + '\tReady', 5000)
                else:
                    main.statBar.showMessage('Ready')
            else:
    ...
    def onImageChange(self):
        ...
        self.get_sot_data()
        self.make_fits()
        self.display2()  # show the 2D images of measurement model or residual
        self.plot2_1D()  # show the cross sections, the Aeff-curve or the SOT-curve
    ...
    def get_beam_max(self):
        ...
        if backg + im_to_show2.max() > sat_value * rel_sat_limit:
            self.LbSaturation.setText('<b> <span style="color:#f00;">SATURATED IMAGE</span> </b>')
        else:
            self.LbSaturation.setText('This image is not saturated')
    ...
    def get_backg(self):
        ...
        if np.any(np.isnan(roiS[0])):
            ...
            if analyze_all_files_check:
                main.statBar.showMessage(statusbarmessage + '\tThe ROI is not defined', 5000)
            else:
    ...
    def get_sot_data(self):
        ...
        if self.cbMakeSotCalc.isChecked():  # calculates additionally the sot for a fixed threshold
            soft_val = (im_to_show2 / b_max[2] > self.sb_sot_thresh.value()).sum()
            self.LbSotResult.setText(
                f'The SOT is: {soft_val} px²')
    ...
    def plot2_1D(self):
        ...
        if self.Cob1Ddisp.currentText() == "Cross sections":
            ...
            YY1 = np.array(range(im_to_show2.shape[0]))  # the line numbers
            XX1 = np.ones(len(YY1)) * round(b_max[0])  # the corresponding col no

            # plot model data: horizontal cross sections
            XX2 = np.array(range(im_to_show2.shape[1]))  # the col numbers
            YY2 = np.ones(len(XX2)) * round(b_max[1])  # the corresponding line no

            axPlt2.plot(YY1, gauss2D_cst_offs.f(XX1, YY1, paras_ro),
                        '--g', label="round G. vert.")
            axPlt2.plot(XX2, gauss2D_cst_offs.f(XX2, YY2, paras_ro),
                        '-.', color="red", label="round G. horiz.")

            axPlt2.plot(YY1, gaussEll2D_cst_offs.f(XX1, YY1, paras_el),
                        '--', color="cyan", label="ell. G. vert.")
            axPlt2.plot(XX2, gaussEll2D_cst_offs.f(XX2, YY2, paras_el),
    ...
    def display2(self):
        ...
        elif self.Cob2Ddisp.currentText() == "Elliptic G. model":
            XX, YY = np.meshgrid(range(im_to_show2.shape[1]), range(im_to_show2.shape[0]))
            try:
                col_idx = 2
                paras = [b_fits[0][col_idx]["max"], b_fits[0][col_idx]["x_0"],
                ...
                mod_z = gaussEll2D_cst_offs.f(XX, YY, paras)
                self.show_image2(im=mod_z, vmi2=measurement.min(), vma2=measurement.max())
            except:
                pass

                """col_idx == 2:  # elliptic gau: gaussEll2D_cst_offs
            # paras = [vert_factor, cent_x, cent_y, waist_rad_a, waist_rad_b, theta]
            p_ini = [b_fits[0][col_idx]["max"] * b_max[2],  # because the max is relative
                     b_fits[0][col_idx]["x_0"],
                     b_fits[0][col_idx]["y_0"],
        ...
        elif self.Cob2Ddisp.currentText() == "Elliptic G. residual (measurement - model)":
            ti_mod = ti.time()
            XX, YY = np.meshgrid(range(im_to_show2.shape[1]), range(im_to_show2.shape[0]))
            # try:
            col_idx = 2
    ...
    def UpdateLinesIm2(self, im=None):
        ...
        try:
            for line in axIM2.get_lines():  # ax.lines:
                line.remove()
        except NameError:  # just a guess (that seems to work)
            pass
    ...
    def make_result_dict(self):
        ...
        res_soft = np.ones(len(good_idx)) * np.nan  # SOT for fixed threshold
        ...
        if analyze_all_files_check:
            main.statBar.showMessage(statusbarmessage + '\tAnalyzing stack', 5000)
        else:
            main.statBar.showMessage("Analyzing stack")
        ...
        for idx in range(len(good_idx)):
            ...
            if self.cbMakeSotCalc.isChecked():
                res_soft[idx] = soft_val
        ...
        if self.TbFit2.cellWidget(0, 2).currentText() == "Yes":  # Elliptic Gauss fit done
            ...
            results["EllG beam radius w2 (px)"] = np.array([res_b_fit[row, 2]["w2"] for row in range(len(good_idx))])
        ...
        CobPl_li = [self.Cob4Pl31, self.Cob4Pl32, self.Cob4Pl33, self.Cob4Pl34]
        key_li = list(results.keys())
        print("key_li = ", key_li)
    ...
    def show_res_statistics_table(self, ti_stack_0):
        ...
        for col_idx in range(len(key_li)):  # the names can be reused but new instances need to be created
            ...
            # check if new entry
            if key_li[col_idx] not in all_tab4_cobo_items:
                self.Cob4Pl1.addItem(key_li[col_idx])
                self.Cob4Pl2.addItem(key_li[col_idx])
                self.Cob4Pl3.addItem(key_li[col_idx])
                self.Cob4Pl4.addItem(key_li[col_idx])
                all_tab4_cobo_items.append(key_li[col_idx])
    ...
    def set_checkb_state4(self):
        # print("set_checkb_state4 called")
        cobPl_li_4 = [self.Cob4Pl1, self.Cob4Pl2, self.Cob4Pl3, self.Cob4Pl4]
        list_Cb = [self.Cb4Pl1, self.Cb4Pl2, self.Cb4Pl3, self.Cb4Pl4]
        for Cob, Cb in zip(cobPl_li_4, list_Cb):
            if self.sender() is Cob:
                if Cob.currentText() == "Effective surface (px²)":
                    Cb.setEnabled(True)
                else:
                    Cb.setEnabled(False)
        self.plot4_1D()

    def set_checkb_state4_2(self):
        # print("set_checkb_state4_2 called")
        if self.sender() is self.CobDataOrigin:
            if self.CobDataOrigin.currentText() == "Process image files with identical parameters":
                self.CbExport.setEnabled(True)
            else:
                self.CbExport.setEnabled(False)


    def plot4_1D(self):
        # print("plot4_1D called")

        # create all z-plots depending on the situation of the combo boxes and the
        # checkboxes (if relevant)
        global axPlt_li4, cobPl_li_4

        # clear figure
        self.figurePlt4.clear()
        axPlt41 = self.figurePlt4.add_subplot(221)
        axPlt42 = self.figurePlt4.add_subplot(222)
        axPlt43 = self.figurePlt4.add_subplot(223)
        axPlt44 = self.figurePlt4.add_subplot(224)
        axPlt_li4 = [axPlt41, axPlt42, axPlt43, axPlt44]
        col_li = ['green', 'grey', 'red', 'blue']

        cobPl_li_4 = [self.Cob4Pl1, self.Cob4Pl2, self.Cob4Pl3, self.Cob4Pl4]
        cb_li_4 = [self.Cb4Pl1, self.Cb4Pl2, self.Cb4Pl3, self.Cb4Pl4]

        if self.Cob4Pl1.currentText() == "":
            return  # nothing to plot fct was called before processing input data

        for (this_ax, this_col, this_cobo, this_cb) in zip(axPlt_li4, col_li, cobPl_li_4, cb_li_4):
            curr_text = this_cobo.currentText()
            if not type(results[curr_text]) is np.ndarray:
                continue  # do not do what follows

            if np.any(np.isnan(results[curr_text])):
                continue  # do not do what follows

            y_list = []
            y_err_list = []
            for z_idx in range(len(final_results_list)):
                y_list.append(final_results_list[z_idx][curr_text].mean())
                y_err_list.append(final_results_list[z_idx][curr_text].std())

            idx_sorted = np.array(list_z_positions).argsort()
            z = np.array(list_z_positions)[idx_sorted].copy()
            y = np.array(y_list)[idx_sorted]
            y_err = np.array(y_err_list)[idx_sorted]

            this_ax.set_ylabel(this_cobo.currentText())  # standard (overridden by "eff. rad" checkbox)
            # See if Eff radius is wanted
            if this_cobo.currentText() == "Effective surface (px²)" and this_cb.isChecked():  # action on tick
                #  For Gaussian beam only: F = 2 Ep / (pi w²) --> Aeff = pi w² /2 --> w = sqrt(2 Aeff/pi)
                y = np.sqrt(2 * y / np.pi)  # make it w from Aeff
                y_err = y_err / np.sqrt(2 * np.pi * y)  # make it w_err from Aeff_err
                # dw/dA = sqrt(2/pi) * 1/2 * 1/sqrt(A)
                this_ax.set_ylabel("Effective G beam radius (px)")

            if this_cobo.currentText() == "Energy (GL)":
        ...
        # refresh canvas
        self.figurePlt4.tight_layout()
        self.canvasPlt4.draw()


    def select_data_origin(self):
        """
        Launch the functions that fill the global variables
        list_z_positions, final_results_list
        :return:
        """
        #self.CobDataOrigin.addItems(["Process image files with identical parameters",
        #                            "Load data from Excel files exported earlier"
    ...
    def analyze_all_files(self):
        global f_name, list_z_positions, final_results_list, thisFile_type, list_files
        global analyze_all_files_check, statusbarmessage, dirname2, baseName2, baseName

        final_results_list = []
        all_tab4_cobo_items = []

        # TODO: Show pop up window to stop the processing
        analyze_all_files_check = True
        def extract_z_from_filename(fi):
            z = -20e20
            # extract the position floating point number from the string fi
            rr = re.findall(r"[-+]?[.]?[\d]+(?:,\d\d\d)*[\.|\,|p]?\d*(?:[eE][-+]?\d+)?", fi)
            zz = rr[0]  # The first number should be the z-position, other numbers may follow and are ignored
            zz = zz.replace(",", ".")
            zz = zz.replace("p", ".")
            z = float(zz)
            return z
        ...
        fi_size = os.path.getsize(f_name)
        list_files = []  # just for short output of the filenames
        list_z_positions = []
        list_f_names = []
        for filename in os.listdir(dirname):  # just names in clicked dir
            ...
            if thisFile_type != file_type:  # wrong file type
                continue
            if thisFile_type == "wcf":
                thisFileSize = os.path.getsize(os.path.join(dirname, filename))
                ...
                if z_pos == -20e20:  # or error in position extraction
                    continue
                list_files.append(thisBaseName)  # just for short output of the filenames
                list_z_positions.append(z_pos)
                list_f_names.append(os.path.join(dirname, filename))  # full path for loading the files afterwards
            else:  # thisFile_type == "tiff"
                outer_dir = os.path.split(dirname)[0]
                list_subfolders_with_paths = [outer_dir + '/' + f.name for f in os.scandir(outer_dir) if f.is_dir()]
                list_subfolders = []
                for i in list_subfolders_with_paths :
                    name = i.split('/')[-1]
                    try:
                        z_pos = extract_z_from_filename_for_tiff(name)
                    except:
                        continue
                    list_subfolders.append(name)
                    list_files.append(name)  # just for short output of the filenames
                    list_z_positions.append(z_pos)
                    for im_fi in os.listdir(i):
                        if (im_fi.endswith(".tif") or im_fi.endswith(".tiff")
                                or im_fi.endswith(".jpg")or im_fi.endswith(".jpeg")
                                or im_fi.endswith(".png")):
                            list_f_names.append(i + '/' + im_fi)  # full path for loading the files afterwards
                            break  # stop searching for image file (one was found)
                break  # exit filename loop in initial directory
                #TODO restructure to avoid double break

        # Make the processing and save in final_results_list
        final_results_list = []
        if file_type == "wcf":
            for i in range(len(list_f_names)):
                # print("File: ", list_files[i], '; z-val : ', list_z_positions[i])
                f_name = list_f_names[i]
                # main.statBar.showMessage("", 3000)
                text1 = "Analyzing file {} out of {}: "
                statusbarmessage = text1.format(i+1, len(list_f_names))
                self.load_stack()
                self.make_result_dict()
                if self.CbExport.isChecked():
                    baseName = list_files[i]
                    self.export3()
                final_results_list.append(results)
        elif (file_type == "tif" or file_type == "tiff" or file_type == "png" or
              file_type == "jpg" or file_type == "jpeg"):  # if thisFile_type == "tiff" (all other file types)
            for i in range(len(list_f_names)):
                dirname2 = outer_dir
                f_name = list_f_names[i]
                text1 = "Analyzing stack {} out of {} (z = {:.2f}) "
                statusbarmessage = text1.format(i + 1, len(list_f_names), list_z_positions[i])
                self.load_stack()
                self.make_result_dict()
                if self.CbExport.isChecked():
                    baseName2 = "z = " + str(list_z_positions[i]) + " - "
                    self.export3()
                final_results_list.append(results)

        analyze_all_files_check = False
        self.tabs.setCurrentIndex(3)  # Go back to the z-variation tab
        self.set_checkb_state4()  # make plots
